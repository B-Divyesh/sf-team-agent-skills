...
async function publishSkill(event:SubmitEvent) {
  ...
  else try{if(!skill.source_path){notice='Add the JSON package path committed to GitHub.';render();return;}const created=await (await api('/api/skills',{method:'POST',body:JSON.stringify({git_url:skill.git_url,git_commit:skill.git_commit,source_path:skill.source_path})})).json();skills.unshift(created);notice=`Save one-time reviewer key ${created.reviewer_key}. Share only that key with the reviewer.`;}catch(error){notice=message(error);render();return;}
