...
function publishForm() {
  ...
  <label>Private Git credential reference<input name="git_credential_ref" maxlength="64" pattern="[A-Z][A-Z0-9_]*" aria-describedby="git-credential-help"></label><small id="git-credential-help">Optional. The deployment owner maps this uppercase reference to a GitHub credential. It is bound to this source repository only.</small>
  <label class="wide">Skill package JSON path<input name="source_path" maxlength="240" placeholder="skills/secure-commit.json" aria-describedby="source-help"><small id="source-help">Real releases load every signed field from this file at the commit.</small></label>
  <label>Assigned repositories<input name="repositories" required value="atlas-api" aria-describedby="repos-help"><small id="repos-help">Separate repository names with commas.</small></label><label>Pilot repositories<input name="pilot_repositories" value="atlas-api" aria-describedby="pilot-help"><small id="pilot-help">Only these repositories install during pilot.</small></label>
...
async function publishSkill(event:SubmitEvent) {
  ...
  const skill:Skill={id:`skill-${crypto.randomUUID()}`,name:String(form.get('name')),version:String(form.get('version')),summary:String(form.get('summary')),owner:String(form.get('owner')),targets,ring:'draft',updated:'Just now',secrets:csv('secrets'),instructions:String(form.get('instructions')),adapters:Object.fromEntries(targets.map(target=>[target,String(form.get(`adapter:${target}`))])),git_url:String(form.get('git_url')),git_commit:String(form.get('git_commit')),source_path:String(form.get('source_path')),package_digest:'pending',repositories:csv('repositories'),pilot_repositories:csv('pilot_repositories'),approved_by:null,approved_at:null};
  if(isDemo()){skill.package_digest=crypto.randomUUID().replaceAll('-','').padEnd(64,'0');skills.unshift(skill);saveDemo();}
  else try{if(!skill.source_path){notice='Add the JSON package path committed to GitHub.';render();return;}const gitCredentialRef=String(form.get('git_credential_ref')||'').trim();if(gitCredentialRef)await api('/api/git-credentials',{method:'POST',body:JSON.stringify({reference:gitCredentialRef,git_url:skill.git_url})});const created=await (await api('/api/skills',{method:'POST',body:JSON.stringify({git_url:skill.git_url,git_credential_ref:gitCredentialRef||undefined,git_commit:skill.git_commit,source_path:skill.source_path})})).json();skills.unshift(created);notice=`Save one-time reviewer key ${created.reviewer_key}. Share only that key with the reviewer.`;}catch(error){notice=message(error);render();return;}
